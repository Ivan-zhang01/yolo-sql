...
class MY_Controller extends CI_Controller
{
    ...
    /* --------------------------------------------------------------
     * CUSTOM FUNCTIONALITY ADDITIONAL TO ORIGINAL SOURCE
     * ------------------------------------------------------------ */
     ...
     */
    public function authenticate($user_type = '', $return_on_fail = '')
    {
		if (isset($this->require_logged_in))
        {
			if (!$this->session_model->is_logged_in())
            {
                // store the calling URI in the session so we can return the user to that page upon successful login
                $this->session->set_userdata('return_uri_on_login', $this->uri->uri_string());

				$this->session->set_flashdata('error', 'You need to be logged in to access the system');
				redirect('login');
			}
            else
            {
                if ($user_type != '')
                {
                    if ($this->session_model->current_user()->user_type == $user_type)
                    {
                        $this->user_id = $this->session_model->current_user()->id;

                        if ($this->session->userdata('return_uri_on_login') != '')
                        {
                            $return_uri = $this->session->userdata('return_uri_on_login');
                            $this->session->unset_userdata('return_uri_on_login');
                    ...
                    {
                        $this->session->set_flashdata('error', 'You do not have sufficient access to this facility');
                        redirect($return_on_fail);
                    }
                }
                else
                {
    ...
    /**
     * Used to check whether the user is authenticated for the admin system
     */
    public function authenticate_admin_area($user_type = '', $return_on_fail = '')
    {
		if (isset($this->require_logged_in))
        {
			if (!$this->session_model->is_logged_in())
            {
				...
				$this->session->set_flashdata('message', array('type' => 'error', 'content' => 'You need to be signed in to access the system.'));
				redirect('login');
			}
            else
            {
                if ($this->session_model->current_user()->user_type != 'M' && $this->session_model->current_user()->user_type != 'A')
                {
                    $this->session->set_flashdata('message', array('type' => 'error', 'content' => 'You do not have sufficient access to this part of the system.'));
                    redirect('login');
                }
                else
                {
                    if ($user_type != '')
                    {
                        ...
                        {
                            $this->session->set_flashdata('message', array('type' => 'error', 'content' => 'You do not have sufficient access to this facility.'));
                            redirect($return_on_fail);
                        }
                    }
                    else
